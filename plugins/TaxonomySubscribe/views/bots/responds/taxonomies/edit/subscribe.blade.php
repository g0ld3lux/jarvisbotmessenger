...
@section('form')
        ...
        <div class="col-md-6">
            <select class="form-control" name="channel">
                ...
                @foreach($project->subscriptionsChannels()->ordered()->get() as $channel)
                    <option value="{{ $channel->id }}" @if(old('channel', $taxonomy->getParamValue('channel')) == $channel->id) selected @endif>{{ $channel->name }}</option>
                @endforeach
            </select>
    ...
    <div class="form-group{{ $errors->has('option') ? ' has-error' : '' }}">
        ...
        <div class="col-md-6">
            <div class="radio">
                <label>
                    <input type="radio" name="option" id="optionAdd" value="add" {{ old('option', $taxonomy->getParamValue('option')) == 'add' ? 'checked' : '' }}>
                    Add recipient to subscription channel
                </label>
            </div>
            <div class="radio">
                <label>
                    <input type="radio" name="option" id="optionRemove" value="remove" {{ old('option', $taxonomy->getParamValue('option')) == 'remove' ? 'checked' : '' }}>
