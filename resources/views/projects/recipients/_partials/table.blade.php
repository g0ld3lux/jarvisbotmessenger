    ...
    <tbody>
        @foreach($recipients as $recipient)
            <tr @if($recipient->chat_disabled) class="text-muted" @endif>
                ...
                <td>{{ $recipient->created_at ? adjust_project_timezone($project, $recipient->created_at)->format('F j, Y, g:i A') : '' }}</td>
                <td>{{ $recipient->updated_at ? adjust_project_timezone($project, $recipient->updated_at)->format('F j, Y, g:i A') : '' }}</td>
                <td class="text-right">
